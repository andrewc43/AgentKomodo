...
def browse_website(url: str, question: str) -> str:
    """Browse a website and return an answer + a few links."""
    driver: WebDriver | None = None
    try:
        driver, text = scrape_text_with_selenium(url)

        # Optional overlay, but don't fail if missing or blocked
        try:
            add_header(driver)
        except Exception:
            pass
        ...
        links: List[str] = []
        try:
            links = _with_timeout(lambda: scrape_links_with_selenium(driver)[:5], timeout_s=5, default=[])
        except Exception:
            links = []
...
def close_browser(driver: WebDriver) -> None:
    ...
    # Quit in a thread and don't let it block forever
...
def _with_timeout(fn, timeout_s: int, default):
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
        fut = ex.submit(fn)
        try:
